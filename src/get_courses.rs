...
async fn get_course(link: &str, client: &Client) -> Result<Course> {
    ...
    let mut da_tokens = element
        ...
        .split(' ')
        .into_iter()
        .filter(|s| !s.is_empty() && !s.contains("student") && *s != "allowed" && *s != "only")
        .collect::<Vec<&str>>();
    let mut prereq_tokens = da_tokens.iter().peekable();

    let mut prerequisites: Vec<Token> = vec![];
    while let Some(current) = prereq_tokens.next() {
        match *current {
            "or" => prerequisites.push(Token::Logical(Logic::Or)),
            "and" => prerequisites.push(Token::Logical(Logic::And)),
            "(" => prerequisites.push(Token::Logical(Logic::GroupStart)),
            ")" => prerequisites.push(Token::Logical(Logic::GroupEnd)),
            "graduate" | "grad" => prerequisites.push(Token::Seniority(Seniority {
                freshman: false,
                sophomore: false,
                junior: false,
                senior: false,
            ...
            "at" => {
                if *prereq_tokens.next().context("")? == "least" {
                    // TODO Add error message
                    match **prereq_tokens.peek().context("")? {
            ...
            "no" => {
                ...
                prereq_tokens.skip(4);
            }
            "coreq" => {
                prerequisites.push(Token::CourseCoreq(
                    (*prereq_tokens.next().context("")?).to_owned()
                        + " "
                        + prereq_tokens.next().context("")?,
                ));
            }
            "pre-/co-req" => {
                let course = (*prereq_tokens.next().context("")?).to_owned()
                    + " "
                    + prereq_tokens.next().context("")?;
                prerequisites.push(Token::Logical(Logic::GroupStart));
            ...
            "need" => match *prereq_tokens.nth(4).context("")? {
            ...
            _ => {
                if current.len() == 3 {
                    let course = (*current).to_owned() + " " + prereq_tokens.next().context("")?;
                    if *prereq_tokens.next().context("")? == "coreq" {
                        prerequisites.push(Token::CourseCoreq(course));
                    } else {
                        prerequisites.push(Token::CoursePrereq(course));
                    }
                    ...
                    prerequisites.push(Token::Seniority(Seniority {
                        ...
                        graduate: true,
                        doctorate: true,
                        major: Some((*current).to_owned()),
                    }))
                } else {
                    bail!("encountered unexpected token \"{}\"", current);
                }
            }
        };
    }
